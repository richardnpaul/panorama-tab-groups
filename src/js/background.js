...
async function setActionTitle(windowId, activeGroup = null) {
  ...
  if (activeGroup === null) {
    activeGroup = await browser.sessions.getWindowValue(
      windowId,
      'activeGroup',
    );
  }
...
async function changeActiveGroupBy(offset) {
  ...
  let activeGroup = await browser.sessions.getWindowValue(
    windowId,
    'activeGroup',
  );
...
async function tabActivated(activeInfo) {
  ...
  const activeGroup = await browser.sessions.getTabValue(
    activeInfo.tabId,
    'groupId',
  );
...
async function salvageGrouplessTabs() {
  ...
  await Promise.all(
    tWindows.map(async (window) => {
      ...
      windows[window.id].groups = await browser.sessions.getWindowValue(
        window.id,
        'groups',
      );
    }),
